 ...
 * @since 1.0.0
 */
class Stea_GeneratePress_Compatibility {

	/**
	 * Instance of Stea_GeneratePress_Compatibility
	 *
	 * @var Stea_GeneratePress_Compatibility
	 */
	...
	public static function instance() {
		if ( ! isset( self::$instance ) ) {
			self::$instance = new Stea_GeneratePress_Compatibility();
	...
	public function hooks() {
		...
		if ( stea_theme_builder_header_enabled() ) {
			...
			add_action( 'generate_header', 'stea_theme_builder_render_header' );
		}

		if ( stea_theme_builder_footer_enabled() ) {
			remove_action( 'generate_footer', 'generate_construct_footer_widgets', 5 );
			remove_action( 'generate_footer', 'generate_construct_footer' );
			add_action( 'generate_footer', 'stea_theme_builder_render_footer' );
		}

		if ( stea_theme_builder_is_singular_enabled() ) {

			update_post_meta( stea_theme_builder_get_singular_id(), '_generate-sidebar-layout-meta', 'no-sidebar' );
			update_post_meta( stea_theme_builder_get_singular_id(), '_generate-full-width-content', 'true' );
			update_post_meta( stea_theme_builder_get_singular_id(), '_generate-disable-headline', 'true' );
		...
		if ( stea_theme_builder_is_archive_enabled() ) {
			...
			update_post_meta( stea_theme_builder_get_archive_id(), '_generate-sidebar-layout-meta', 'no-sidebar' );
			update_post_meta( stea_theme_builder_get_archive_id(), '_generate-full-width-content', 'true' );
			update_post_meta( stea_theme_builder_get_archive_id(), '_generate-disable-headline', 'true' );
	...
	public function generate_do_page_container_classes( $classes ) {
		$classes   = array();
		$classes[] = 'st-elementor-addons-wrapper';
...
Stea_GeneratePress_Compatibility::instance();
