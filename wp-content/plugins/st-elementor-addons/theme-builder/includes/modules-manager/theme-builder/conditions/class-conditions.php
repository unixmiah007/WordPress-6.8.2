...
namespace ST_Elementor_Addons\ModulesManager\Theme_Builder\Conditions;
...
/**
 * STEA_Conditions Class
 */
class STEA_Conditions {
	use Singleton;
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_stea_hfe_get_posts_by_query', array( $this, 'stea_hfe_get_posts_by_query' ) );
	}
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'stea_hf_display_condition_post_types', array_merge( $post_types, $custom_post_type ) );

		$special_pages = array(
			'special-404'    => __( '404 Page', 'st-elementor-addons' ),
			'special-search' => __( 'Search Page', 'st-elementor-addons' ),
			'special-blog'   => __( 'Blog / Posts Page', 'st-elementor-addons' ),
			'special-front'  => __( 'Front Page', 'st-elementor-addons' ),
			'special-date'   => __( 'Date Archive', 'st-elementor-addons' ),
			'special-author' => __( 'Author Archive', 'st-elementor-addons' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'st-elementor-addons' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'st-elementor-addons' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'st-elementor-addons' ),
					'basic-singulars' => __( 'All Singulars', 'st-elementor-addons' ),
					'basic-archives'  => __( 'All Archives', 'st-elementor-addons' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'st-elementor-addons' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'st-elementor-addons' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'st-elementor-addons' ),
			),
		);

		// Filter options displayed in the display conditions select field.
		return apply_filters( 'stea_hf_display_conditions_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'st-elementor-addons' ),
				'value' => array(
					'all'        => __( 'All', 'st-elementor-addons' ),
					'logged-in'  => __( 'Logged In', 'st-elementor-addons' ),
					'logged-out' => __( 'Logged Out', 'st-elementor-addons' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'st-elementor-addons' ),
				'value' => array(),
			),
		);
		...
		return apply_filters( 'stea_hf_display_user_roles_list', $selection_options );
	}
	 ...
	 * @return void
	 */
	public function stea_hfe_get_posts_by_query() {

		check_ajax_referer( 'stea-hfe-get-posts-by-query', 'nonce' );
		...
		unset( $post_types['stea-theme-template'] ); // Exclude EHF templates.
	 ...
	 * This is only run from stea_hfe_get_posts_by_query()
	 *
	 * @access public
	 *
	 * @since 1.3.0
	...
	public function admin_styles() {
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( STEA_PATH . 'theme-builder/assets/lib/select2/i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$lang = $select2_lang[ $wp_lang ];
			}
		}

		wp_register_script(
			'stea-hf-display-conditions',
			STEA_URL . 'theme-builder/admin/assets/js/stea-theme-display-conditions.js',
			array(
				'jquery',
				'stea-select2',
			),
		...
		wp_enqueue_script( 'stea-hf-display-conditions' );

		wp_register_script(
			'stea-hf-display-conditions-user-role',
			STEA_URL . 'theme-builder/admin/assets/js/stea-theme-display-conditions-user-role.js',
			array(
				'jquery',
			),
			STEA_VERSION,
		...
		wp_enqueue_script( 'stea-hf-display-conditions-user-role' );

		wp_register_style( 'stea-hf-display-conditions-style', STEA_URL . 'theme-builder/admin/css/stea-theme-display-conditions.css', array(), STEA_VERSION );
		wp_enqueue_style( 'stea-hf-display-conditions-style' );

		$localize_vars = array(
			'stea_lang'     => $lang,
			'please_enter'  => __( 'Please enter', 'st-elementor-addons' ),
			'please_delete' => __( 'Please delete', 'st-elementor-addons' ),
			'more_char'     => __( 'or more characters', 'st-elementor-addons' ),
			'character'     => __( 'character', 'st-elementor-addons' ),
			'loading'       => __( 'Loading more results…', 'st-elementor-addons' ),
			'only_select'   => __( 'You can only select', 'st-elementor-addons' ),
			'item'          => __( 'item', 'st-elementor-addons' ),
			'char_s'        => __( 's', 'st-elementor-addons' ),
			'no_result'     => __( 'No results found', 'st-elementor-addons' ),
			'searching'     => __( 'Searching…', 'st-elementor-addons' ),
			'not_loader'    => __( 'The results could not be loaded.', 'st-elementor-addons' ),
			'search'        => __( 'Search pages / post / categories', 'st-elementor-addons' ),
			'ajax_nonce'    => wp_create_nonce( 'stea-hfe-get-posts-by-query' ),
		);
		wp_localize_script( 'stea-select2', 'stea_display_conditions', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Display On Condition', 'st-elementor-addons' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-stea-hf-display-conditions-' . $rule_type . '-condition">';
		$output .= '<div class="stea-hf__display-condition stea-hf__display-condition-{{data.id}}" data-rule="{{data.id}}" >';
		$output .= '<span class="stea-hf__display-condition-delete dashicons dashicons-dismiss"></span>';
		/* Condition Selection */
		$output .= '<div class="stea-hf__display-condition-wrapper" >';
		$output .= '<select name="' . esc_attr( $input_name ) . '[rule][{{data.id}}]" class="stea-hf__display-condition-input form-control stea-hf-input">';
		$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
		...
		$output .= '</div> <!-- stea-hf__display-condition -->';

		/* Specific page selection */
		$output .= '<div class="stea-hf__display-condition-specific-page-wrapper" style="display:none">';
		$output .= '<select name="' . esc_attr( $input_name ) . '[specific][]" class="stea-hf__display-condition-select2 stea-hf__display-condition-specific-page form-control stea-hf-input " multiple="multiple">';
		$output .= '</select>';
		$output .= '</div>';
		...
		/* Wrapper Start */
		$output .= '<div class="stea-hf__display-condition-container stea-hf__display-condition-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
		$output .= '<div class="stea-hf-display-condition-selector-wrapper stea-hf-display-condition-' . $rule_type . '-on">';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'st-elementor-addons' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' !== $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'st-elementor-addons' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'st-elementor-addons' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		$output = '<div class="stea-hf__display-condition-builder-wrapper">';
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="stea-hf__display-condition stea-hf__display-condition-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="stea-hf__display-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="stea-hf__display-condition-wrapper" >';
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . $index . ']" class="stea-hf__display-condition-input form-control stea-hf-input">';
			$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
		...
		/* Add new rule */
		$output .= '<div class="stea-hf__add-include-display-condition-wrapper">';
		$output .= '<a href="#" class="button" data-rule-id="' . absint( $index ) . '" data-rule-type="' . $type . '">' . $add_rule_label . '</a>';
		$output .= '</div>';

		if ( 'display' === $type ) {
			/* Add new rule */
			$output .= '<div class="stea-hf__add-exclude-display-condition-wrapper">';
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Condition', 'st-elementor-addons' ) . '</a>';
			$output .= '</div>';
		}
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		$class          = isset( $settings['class'] ) ? $settings['class'] : '';
		$rule_type      = isset( $settings['rule_type'] ) ? $settings['rule_type'] : 'target_rule';
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'st-elementor-addons' );
		$saved_values   = $value;
		$output         = '';
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-stea-hf-user-role-condition">';
			$output     .= '<div class="stea-hf__user-role-condition stea-hf__user-role-{{data.id}}" data-rule="{{data.id}}" >';
				$output .= '<span class="stea-hf__user-role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="stea-hf__user-role-condition-wrapper" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[{{data.id}}]" class="stea-hf__user-role-condition-input form-control stea-hf-input">';
					$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- stea-hf__user-role-condition -->';
		$output             .= '</script>';
		...
		$output         .= '<div class="stea-hf__user-role-wrapper stea-hf__user-role-display-on-wrap" data-type="display">';
			$output     .= '<div class="stea-hf__user-role-selector-wrapper stea-hf__user-role-display-on">';
				$output .= '<div class="stea-hf__user-role-builder-wrapper">';
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="stea-hf__user-role-condition stea-hf__user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="stea-hf__user-role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="stea-hf__user-role-condition-wrapper" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[' . $index . ']" class="stea-hf__user-role-condition-input form-control stea-hf-input">';
					$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
			...
			foreach ( $selection_options as $group => $group_data ) {
				...
				$output .= '</div>';
				/* Add new rule */
				$output .= '<div class="stea-hf__user-add-role-condition-wrapper">';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'stea_hf_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
