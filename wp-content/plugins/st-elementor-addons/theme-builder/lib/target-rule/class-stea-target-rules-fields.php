<?php
/**
 * Stea_Target_Rules_Fields
 *
 * @package   Stea
 */

namespace Stea_Theme_Builder\Lib;

/**
 * Meta Boxes setup
 */
class Stea_Target_Rules_Fields {
	...
	public function __construct() {
		define( 'STEA_TARGET_RULE_URI', plugins_url( '/', __FILE__ ) );
		define( 'STEA_TARGET_RULE_VER', '1.0.0' );
		define( 'STEA_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );

		add_action( 'admin_action_edit', array( $this, 'initialize_options' ) );
		add_action( 'wp_ajax_stea_theme_builder_get_posts_by_query', array( $this, 'stea_theme_builder_get_posts_by_query' ) );
	}
	...
	public static function get_location_selections() {
		...
		$post_types = apply_filters( 'stea_location_rule_post_types', array_merge( $post_types, $custom_post_type ) );
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'stea_display_on_list', $selection_options );
	}
	...
	public static function get_user_selections() {
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'stea_user_roles_list', $selection_options );
	}
	 ...
	 * @since  1.0.0
	 */
	public function stea_theme_builder_get_posts_by_query() {

		check_ajax_referer( 'st-elementor-addons-get-posts-by-query', 'nonce' );
		...
		unset( $post_types['stea-theme-template'] );
	 ...
	 * This is only run from stea_theme_builder_get_posts_by_query()
	 *
	...
	public function admin_styles() {
		wp_enqueue_script( 'stea-select2', STEA_TARGET_RULE_URI . 'select2.js', array( 'jquery' ), STEA_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( STEA_TARGET_RULE_DIR . 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
				$stea_lang = $select2_lang[ $wp_lang ];
				wp_enqueue_script(
					'stea-select2-lang',
					STEA_TARGET_RULE_URI . 'i18n/' . $select2_lang[ $wp_lang ] . '.js',
					array(
						'jquery',
						'stea-select2',
					),
					STEA_TARGET_RULE_VER,
					true
				);
			}
		}
		...
		wp_register_script(
			'stea-target-rule',
			STEA_TARGET_RULE_URI . 'target-rule.js',
			array(
				'jquery',
				'wp-util',
				'stea-select2',
		...
		wp_enqueue_script( 'stea-target-rule' );

		wp_register_script(
			'stea-user-role',
			STEA_TARGET_RULE_URI . 'user-role.js',
			array(
				'jquery',
			),
			STEA_TARGET_RULE_VER,
		...
		wp_enqueue_script( 'stea-user-role' );

		wp_register_style( 'stea-select2', STEA_TARGET_RULE_URI . 'select2.css', '', STEA_TARGET_RULE_VER );
		wp_enqueue_style( 'stea-select2' );
		wp_register_style( 'stea-target-rule', STEA_TARGET_RULE_URI . 'target-rule.css', '', STEA_TARGET_RULE_VER );
		wp_enqueue_style( 'stea-target-rule' );
		...
		$localize_vars = array(
			...
			'search'        => __( 'Select Specific', 'st-elementor-addons' ),
			'ajax_nonce'    => wp_create_nonce( 'st-elementor-addons-get-posts-by-query' ),
		);
		wp_localize_script( 'stea-select2', 'SteaRules', $localize_vars );
	}
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output .= '<script type="text/html" id="tmpl-stea-target-rule-' . $rule_type . '-condition">';
		$output .= '<div class="stea-target-rule-condition stea-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
		...
		$output .= '<select name="' . esc_attr( $input_name ) . '[rule][{{data.id}}]" class="target_rule-condition form-control stea-input">';
		...
		$output .= '</div> <!-- stea-target-rule-condition -->';

		/* Specific page selection */
		$output .= '<div class="target_rule-specific-page-wrap d-none">';
		$output .= '<select name="' . esc_attr( $input_name ) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control stea-input " multiple="multiple">';
		$output .= '</select>';
		$output .= '</div>';
		...
		/* Wrapper Start */
		$output .= '<div class="stea-target-rule-wrapper stea-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
		$output .= '<div class="stea-target-rule-selector-wrapper stea-target-rule-' . $rule_type . '-on">';
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) {
			$output .= '<div class="stea-target-rule-condition stea-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . $index . ']" class="target_rule-condition form-control stea-input">';
			$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
		...
		if ( 'display' === $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'st-elementor-addons' ) . '</a>';
			$output .= '</div>';
		}
	...
	public function get_current_layout( $layout_id, $option ) {
		...
		$current_layout = apply_filters( 'stea_target_page_settings', $current_layout, $layout_id );
	...
	public static function target_user_role_settings_field( $name, $settings, $value ) {
		...
		/* WP Template Format */
		$output         .= '<script type="text/html" id="tmpl-stea-user-role-condition">';
			$output     .= '<div class="stea-user-role-condition stea-user-role-{{data.id}}" data-rule="{{data.id}}" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[{{data.id}}]" class="user_role-condition form-control stea-input">';
					$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
		...
		foreach ( $selection_options as $group => $group_data ) {
			...
			$output .= '</optgroup>';
					...
					$output .= '</select>';
				$output     .= '</div>';
			$output         .= '</div> <!-- stea-user-role-condition -->';
		$output             .= '</script>';
		...
		$output         .= '<div class="stea-user-role-wrapper stea-user-role-display-on-wrap" data-type="display">';
			$output     .= '<div class="stea-user-role-selector-wrapper stea-user-role-display-on">';
				$output .= '<div class="user_role-builder-wrap">';
		foreach ( $saved_values as $index => $data ) {
			$output     .= '<div class="stea-user-role-condition stea-user-role-' . $index . '" data-rule="' . $index . '" >';
				$output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
				/* Condition Selection */
				$output     .= '<div class="user_role-condition-wrap" >';
					$output .= '<select name="' . esc_attr( $input_name ) . '[' . $index . ']" class="user_role-condition form-control stea-input">';
					$output .= '<option value="">' . __( 'Select', 'st-elementor-addons' ) . '</option>';
	...
	public function get_posts_by_conditions( $post_type, $option ) {
		...
		if ( is_array( self::$current_page_data ) && isset( self::$current_page_data[ $post_type ] ) ) {
			return apply_filters( 'stea_get_display_posts_by_conditions', self::$current_page_data[ $post_type ], $post_type );
		}
 ...
 */
Stea_Target_Rules_Fields::get_instance();
